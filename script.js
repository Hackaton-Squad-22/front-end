...
function showProgressBar(percentage){
    const progressBar = document.querySelector('[data-progress="circle"]');
    const progressNumber = document.querySelector('[data-progress="number-circcle"]');

    const userProgressString = `${percentage}%`;
    console.log(userProgressString);

    const progressDeg = `${(percentage * 3.6) + 90}deg`;
    
    if(percentage > 0 && percentage <= 50){
        progressBar.style.background = `
            linear-gradient(to right, #CCC 50%, transparent 50%, transparent),
            linear-gradient(${progressDeg}, blue 50%, #CCC 50%)
        `;
    }
    if(percentage > 50){
        progressBar.style.background = `
        linear-gradient(to left, blue 50%, transparent 50%, transparent),
        linear-gradient(${progressDeg}, blue 50%, #CCC 50%)
        `;
    }    
    progressNumber.textContent = `${userProgress}%`;
}